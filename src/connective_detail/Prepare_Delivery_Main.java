...
public class Prepare_Delivery_Main {
	private static PreparedStatement pstmt, pstmt2, pstmt3;
		...
		if (dbConnection.isClosed()) {
			...
			while (retry < maxtry) {
				...
				try {
					...
					MainExecution(dbConnection, lastDate, lastRecord, limitEnd);
					break;
	...
	public static void MainExecution(Connection dbConnection, LocalDate localDatestart, int limitStart, int limitEnd) {
		...
		try {
			while ((localDatestart.isBefore(localDateend) || localDatestart.isEqual(localDateend))) {
				...
				while (rowCount > limitStart) {
					...
					pstmt = dbConnection.prepareStatement(sql_main);
					ResultSet rs = pstmt.executeQuery(sql_main);
					while (rs.next()) {
							...
							try {
								if (!sql_delivered.equals(" ") && (countdelivered == batchsize)) {
									String sql_query = "insert ignore into delivery_mastersstatus values" + ""
											+ sql_delivered + "";
									pstmt3 = dbConnection.prepareStatement(sql_query);
									pstmt3.executeUpdate(sql_query);
							...
							try {
								if (!sql_failed.equals(" ") && (countfailed == batchsize)) {
									...
									String sql_query = "insert ignore into delivery_failedstatus values" + ""
											+ sql_failed + "";
									pstmt2 = dbConnection.prepareStatement(sql_query);
									pstmt2.executeUpdate(sql_query);
									countfailed = 0;
									sql_failed = " ";
									lastDate = localDatestart;
									lastRecord = Sr;
					...
					threadPool.submit(new Runnable() {
						...
						public void run() {
							...
							try {
								...
								{
									MainExecution(dbConnection, localDatestart, limitStart, limitEnd);
								}
